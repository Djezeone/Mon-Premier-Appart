...
const ItemEditModal: React.FC<ItemEditModalProps> = ({
  ...
  const [paidPrice, setPaidPrice] = useState<string>(
    item?.paidPrice !== undefined ? item.paidPrice.toString() : '',
  );
        ...
        <div className="space-y-4 overflow-y-auto pb-4 scrollbar-hide">
          ...
          {roommates.length > 0 && (
            <div className="bg-indigo-50 dark:bg-indigo-900/20 p-3 rounded-xl border border-indigo-100 dark:border-indigo-800">
              <label className="block text-xs font-bold text-indigo-500 uppercase mb-2">
                Qui a payé ?
              </label>
