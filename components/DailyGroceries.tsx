...
import { ArrowLeft, Plus, ShoppingCart, Trash2, Heart, Check, Circle, Apple, Refrigerator, Wine, Sparkles, PlusCircle, X, ChevronDown, ListPlus, Trash } from 'lucide-react';
...
const DailyGroceries: React.FC<{ goBack: () => void }> = ({ goBack }) => {
    const { dailyGroceries, addGroceryItem, toggleGroceryItem, deleteGroceryItem, clearCheckedGroceries, toggleGroceryFavorite } = useInventory();
        ...
        <div className="pb-24 min-h-full dark:text-gray-100 animate-fade-in font-sans">
            ...
            <div className="flex items-center justify-between pt-4 mb-6">
                <div className="flex items-center gap-3">
                    ...
                    <div>
                        ...
                        <p className="text-xs text-emerald-600 dark:text-emerald-400 font-bold">Vie Quotidienne</p>
                    </div>
                </div>
                {checkedCount > 0 && (
                    <button 
            ...
            {totalCount === 0 ? (
                <div className="text-center py-20 px-8">
                    ...
                    <h3 className="text-lg font-bold text-gray-800 dark:text-white">Votre frigo est plein ?</h3>
                    <p className="text-sm text-gray-400 mt-2">Ajoutez des articles pour votre prochaine sortie en magasin.</p>
                </div>
            ) : (
                        ...
                        return (
                            <div key={catKey}>
                                <div className="flex items-center gap-2 mb-3 px-1">
                                    ...
                                    <h3 className="text-sm font-black uppercase tracking-widest text-gray-400">{cat.label}</h3>
                                        ...
                                        >
                                            ...
                                            >
                                                <div className={`w-6 h-6 rounded-full flex items-center justify-center transition-all ${
                                                    ...
                                                    {item.isChecked ? <Check size={14} strokeWidth={4} /> : <Circle size={14} />}
                                                </div>
                                                <div className="flex flex-col">
                                                    <span className={`text-sm font-bold ${item.isChecked ? 'text-gray-400 line-through' : 'text-gray-800 dark:text-gray-200'}`}>
                                                        {item.name}
                                                    </span>
                                                    {item.quantity > 1 && (
                                                        <span className="text-[10px] text-emerald-500 font-bold uppercase">x {item.quantity} {item.unit}</span>
                                                    )}
                                                </div>
                                            </div>
            ...
            <div className="mt-12 bg-emerald-50 dark:bg-emerald-900/20 rounded-2xl p-6 border border-emerald-100 dark:border-emerald-800/50">
                ...
                <p className="text-xs text-emerald-600 dark:text-emerald-300 leading-relaxed">
                    Demandez à l'Assistant dans le Chat : <b>"Fais-moi une liste de courses saine pour 2 personnes avec un budget de 50€"</b>. 
                    Il utilisera l'outil de mise à jour pour remplir cette liste automatiquement !
                </p>
            </div>
        </div>
    );
