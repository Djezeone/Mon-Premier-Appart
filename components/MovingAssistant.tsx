...
import { ArrowLeft, Box, Plus, Minus, Wine, Dumbbell, AlertTriangle, Moon, Coffee, Battery, Flashlight, Sofa } from 'lucide-react';
...
const MovingAssistant: React.FC<MovingAssistantProps> = ({ boxCounts, updateBoxCount, goBack, user, furnitureVolume, setFurnitureVolume }) => {
  ...
  const toggleFlag = (catId: string, type: 'fragile' | 'heavy') => {
      const current = boxCounts[catId] || { count: 0, isFragile: false, isHeavy: false };
      updateBoxCount(catId, current.count, type === 'fragile' ? !current.isFragile : current.isFragile, type === 'heavy' ? !current.isHeavy : current.isHeavy);
  };
        ...
        <div className="animate-fade-in">
            ...
            {difficultyWarning && (
                <div className="bg-red-50 dark:bg-red-900/20 border border-red-200 dark:border-red-800 p-4 rounded-xl mb-4 flex gap-3 items-start">
                    ...
                    <div>
                        ...
                        <p className="text-xs text-red-600 dark:text-red-400">{difficultyWarning.message}</p>
                    </div>
                </div>
            )}
            ...
            <div className="bg-white dark:bg-gray-800 rounded-2xl p-6 shadow-sm border border-gray-100 dark:border-gray-700 mb-6 relative overflow-hidden">
                <div className="flex justify-between items-end mb-2 relative z-10">
                    <div>
                        <h3 className="text-gray-400 font-bold uppercase text-xs tracking-wider mb-1">Véhicule Recommandé</h3>
                        <div className="text-lg font-bold text-gray-800 dark:text-white flex items-center gap-2">
                             <span className="text-2xl">{truckRecommendation.icon}</span> {truckRecommendation.label}
                        </div>
                    </div>
                    <div className="text-right">
                        <div className="text-2xl font-bold text-indigo-600 dark:text-indigo-400">{estimatedVolume} m³</div>
                ...
                <div className="w-full h-4 bg-gray-100 dark:bg-gray-700 rounded-full overflow-hidden mt-4 relative">
                    <div className="absolute top-0 bottom-0 left-0 bg-indigo-500 transition-all duration-1000" style={{ width: `${gaugePercentage}%` }}></div>
                    {/* Tick marks */}
                    <div className="absolute top-0 bottom-0 left-[20%] w-0.5 bg-white/30 border-r border-gray-300/50" title="6m3"></div>
                    <div className="absolute top-0 bottom-0 left-[48%] w-0.5 bg-white/30 border-r border-gray-300/50" title="12m3"></div>
                    <div className="absolute top-0 bottom-0 left-[80%] w-0.5 bg-white/30 border-r border-gray-300/50" title="20m3"></div>
                </div>
            ...
            <div className="bg-white dark:bg-gray-800 rounded-2xl p-4 shadow-sm border border-gray-100 dark:border-gray-700 mb-6">
                ...
                <div className="mb-4">
                    <label className="block text-xs font-bold text-gray-500 uppercase mb-2 flex items-center justify-between">
                         ...
                         <span className="text-indigo-500 font-normal normal-case flex items-center gap-1"><Sofa size={12}/> Canapé ≈ 1.5m³</span>
                    </label>
                ...
                {/* Box Size Selector */}
                <div>
                     <label className="block text-xs font-bold text-gray-500 uppercase mb-2">Taille moyenne des cartons</label>
            ...
            <div className="space-y-3">
                {CATEGORIES.filter(c => c.id !== 'groceries' && c.id !== 'cleaning').map(cat => {
                    ...
                    return (
                        <div key={cat.id} className="bg-white dark:bg-gray-800 p-4 rounded-xl shadow-sm border border-gray-100 dark:border-gray-700">
                            <div className="flex items-center justify-between mb-3">
                                ...
                                <div className="flex items-center gap-3 bg-gray-50 dark:bg-gray-900 rounded-lg p-1">
                                    ...
                                    <span className="w-6 text-center font-bold text-gray-800 dark:text-white">{data.count}</span>
                                    <button 
                                        onClick={() => handleUpdate(cat.id, 1)}
          ...
          <div className="animate-fade-in">
              <div className="bg-indigo-600 rounded-2xl p-6 text-white mb-6">
                  ...
                  <p className="text-indigo-100 text-sm">
                      Ne mets surtout pas ces objets au fond du camion ! Garde-les dans un sac à part pour survivre aux 24 premières heures.
                  </p>
              </div>
              ...
              <div className="grid grid-cols-2 gap-3">
                  {[
                      ...
                      <div key={idx} className="bg-white dark:bg-gray-800 p-4 rounded-xl border border-gray-100 dark:border-gray-700 flex flex-col items-center text-center gap-2 shadow-sm">
