...
const ChatInterface: React.FC<ChatInterfaceProps> = ({
  ...
  const getInventoryContextString = () => {
    ...
    const spent = inventory
      .filter(i => i.acquired)
      .reduce((sum, i) => sum + (i.paidPrice ?? i.estimatedPrice), 0);
  ...
  const executeAnalysis = (filter: string, category?: string) => {
    ...
    const totalCost = items.reduce(
      (acc, i) => acc + (i.acquired ? (i.paidPrice ?? i.estimatedPrice) : i.estimatedPrice),
      0,
    );
    ...
    try {
      ...
      if (currentImage) {
        ...
        const result = await geminiService.analyzeImage(
          imageBase64,
          currentInput || 'Analyse ceci.',
          getInventoryContextString(),
        );
          ...
          if (call.name === 'updateInventory') {
            ...
            if (action === 'add' && Array.isArray(items)) {
              ...
              resultString = JSON.stringify({
                success: false,
                error: 'Unsupported action or missing items',
              });
            }
      ...
      <header className="px-6 py-4 border-b dark:border-gray-800 bg-white dark:bg-gray-800 shadow-sm z-10 flex justify-between items-center">
        <div>
          ...
          <p className="text-xs text-gray-500 dark:text-gray-400">
            Connecté à votre inventaire & administratif
          </p>
        </div>
        <button
          onClick={() => {
            if (window.confirm("Effacer l'historique de discussion ?")) onClearChat();
          }}
      ...
      {selectedImage && (
        <div className="px-4 pb-2 flex items-center gap-2 bg-[#f3f4f6] dark:bg-gray-900">
          <div className="relative group">
            <img
              src={selectedImage}
              className="h-16 w-16 object-cover rounded-lg border border-indigo-200"
              alt="Preview"
            />
            <button
