...
// Fixed ErrorBoundary class to correctly extend Component and ensure inherited properties like 'props' and 'state' are recognized by the TypeScript compiler.
// Adding an explicit constructor ensures that 'props' is properly initialized and available on the class instance.
class ErrorBoundary extends Component<Props, State> {
  // Use a class property to initialize state and also use constructor for compatibility.
  ...
  private handleHardReset = () => {
    if (
      window.confirm(
        "Cela va effacer toutes vos données locales (Inventaire, Chat, etc.) pour réparer l'application. Cette action est irréversible. Continuer ?",
      )
    ) {
      localStorage.clear();
      window.location.reload();
  ...
  public render(): ReactNode {
    // Accessing state inherited from React.Component through this.state.
        ...
        <div className="min-h-screen bg-gray-50 flex flex-col items-center justify-center p-6 text-center font-sans">
          <div className="bg-white p-8 rounded-2xl shadow-xl max-w-md w-full border border-gray-100">
            ...
            <p className="text-gray-500 text-sm mb-6">
              Une erreur inattendue s'est produite. Cela peut arriver lors d'une mise à jour ou d'un
              problème de données.
            </p>
    ...
    // Accessing props inherited from React.Component through this.props.
