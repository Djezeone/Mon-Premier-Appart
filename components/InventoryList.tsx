    ...
    <div className="pb-24 relative min-h-full dark:text-gray-100">
      ...
      <div className="fixed top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2 pointer-events-none z-50">
        {xpPopups.map(popup => (
          <div
            key={popup.id}
            className="animate-float text-xl font-bold text-yellow-500 drop-shadow-md whitespace-nowrap"
          >
            +{popup.amount} XP
          </div>
        ))}
      ...
      <div className="space-y-2">
        {filteredItems.length === 0 ? (
          <div className="text-center py-10 text-gray-400">
            ...
            {filter === 'priority' && (
              <p className="text-sm mt-1">Aucune urgence dans cette vue.</p>
            )}
          </div>
        ) : (
          filteredItems.map(item => {
            ...
            const isPaidByOther =
              item.acquired && item.paidBy && item.paidBy !== 'me' && isRoommateMode;
            const isPaidByMe =
              item.acquired && (!item.paidBy || item.paidBy === 'me') && isRoommateMode;
            ...
            return (
              <div
                key={item.id}
                className={`relative group rounded-xl bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 shadow-sm hover:border-indigo-300 dark:hover:border-indigo-500 transition-all overflow-hidden border-l-4 ${borderClass}`}
              >
                {/* Main Item Row */}
                <div
                  ...
                  <div className="flex-1 flex items-center gap-3">
                    <div
                      className={`w-6 h-6 rounded-full flex-shrink-0 flex items-center justify-center transition-colors ${
                        item.acquired
                          ...
                          : 'bg-gray-100 dark:bg-gray-700 text-gray-300 dark:text-gray-500 group-hover:bg-indigo-100 dark:group-hover:bg-indigo-900 group-hover:text-indigo-400'
                      }`}
                    >
                      {item.acquired ? (
                    ...
                    <div>
                      <div className="flex items-center gap-2 flex-wrap">
                        <p
                          className={`font-medium text-sm sm:text-base ${item.acquired ? 'text-gray-500 dark:text-gray-500 line-through' : 'text-gray-800 dark:text-gray-200'}`}
                        >
                          {item.name}
                        </p>
                  ...
                  <div className="bg-gray-50 dark:bg-gray-900/50 border-t border-gray-100 dark:border-gray-700 p-3 space-y-2 animate-slide-up">
                    {item.subItems!.map(sub => (
                      ...
                      >
                        ...
                        <span
                          className={`text-sm ${sub.acquired ? 'text-gray-400 line-through' : 'text-gray-700 dark:text-gray-300'}`}
                        >
                          {sub.label}
                        </span>
                      </div>
