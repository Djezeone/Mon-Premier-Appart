...
const BudgetCalculator: React.FC<BudgetCalculatorProps> = ({ inventory }) => {
  ...
  const totalSpentReal = inventory
    .filter(i => i.acquired)
    .reduce((sum, item) => {
  ...
  // 3. Remaining
  const totalRemainingEstimated = inventory
    .filter(i => !i.acquired)
  ...
  const totalSharedCost = inventory
    .filter(i => i.acquired)
    .reduce((sum, item) => {
      return sum + (item.paidPrice !== undefined ? item.paidPrice : item.estimatedPrice);
    }, 0);
  ...
  const chartData = CATEGORIES.map(cat => {
    ...
    const catSpent = items
      .filter(i => i.acquired)
      .reduce((sum, i) => sum + (i.paidPrice !== undefined ? i.paidPrice : i.estimatedPrice), 0);
    ...
    <div className="space-y-6 pb-24 dark:text-gray-100">
      ...
      <div className="grid grid-cols-2 gap-4">
        ...
        <div className="bg-white dark:bg-gray-800 text-gray-800 dark:text-white p-5 rounded-2xl shadow-sm border border-gray-100 dark:border-gray-700">
          ...
          <div
            className={`mt-2 text-xs font-medium ${projectedTotal > totalInitialEstimate ? 'text-red-500' : 'text-green-500'}`}
          >
            {projectedTotal > totalInitialEstimate ? '+' : ''}
            {projectedTotal - totalInitialEstimate}€ vs Estimation
          </div>
        </div>
      </div>
        ...
        <div className="bg-purple-50 dark:bg-purple-900/20 border border-purple-100 dark:border-purple-800 rounded-xl p-4 flex items-center justify-between">
          <div>
            <h3 className="text-sm font-bold text-purple-700 dark:text-purple-300">
              Vous emménagez à plusieurs ?
            </h3>
            <p className="text-xs text-purple-500 dark:text-purple-400 mt-1">
              Activez le mode Coloc pour partager les frais.
            </p>
          </div>
      ...
      <div className="space-y-3">
        ...
        {chartData.map(data => (
          ...
          >
            ...
            <div className="text-right">
              <span className="text-sm font-bold block text-gray-800 dark:text-white">
                {data.spent}€
              </span>
