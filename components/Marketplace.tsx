...
const Marketplace: React.FC<MarketplaceProps> = ({ user, goBack }) => {
  const [filter, setFilter] = useState<'all' | 'moving' | 'insurance' | 'energy' | 'internet'>('all');

  const filteredServices = filter === 'all' 
    ? PARTNER_SERVICES 
    : PARTNER_SERVICES.filter(s => s.category === filter);
    ...
    <div className="pb-24 min-h-full dark:text-gray-100">
      <div className="flex items-center gap-3 pt-4 mb-6">
        ...
        <div>
            ...
            <p className="text-xs text-gray-500 dark:text-gray-400">Offres négociées pour les membres Gold</p>
        </div>
      </div>
      ...
      <div className="flex gap-2 overflow-x-auto scrollbar-hide mb-6 pb-2">
          {['all', 'moving', 'insurance', 'energy', 'internet'].map(f => (
              ...
              >
                  {f === 'all' ? 'Tout' : f === 'moving' ? 'Déménagement' : f === 'insurance' ? 'Assurance' : f === 'energy' ? 'Énergie' : 'Internet'}
              </button>
          ))}
      </div>
      ...
      <div className="grid grid-cols-1 gap-4">
          {filteredServices.map(service => (
              <div key={service.id} className="bg-white dark:bg-gray-800 rounded-2xl p-5 shadow-sm border border-gray-100 dark:border-gray-700 relative overflow-hidden group">
      ...
      <div className="mt-8 p-4 bg-gray-50 dark:bg-gray-800/50 rounded-xl border border-dashed border-gray-200 dark:border-gray-700 text-center">
          <p className="text-xs text-gray-400">
              Les liens ci-dessus sont des partenariats affiliés. En passant par ces liens, vous soutenez le développement de l'application sans surcoût pour vous.
          </p>
      </div>
