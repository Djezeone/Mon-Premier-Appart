...
const AppContent: React.FC = () => {
  ...
  return (
    <div className="max-w-md mx-auto min-h-screen bg-[#f3f4f6] dark:bg-gray-900 relative shadow-2xl overflow-hidden font-inter transition-colors">
      ...
      <main
        className="h-full overflow-y-auto px-4 pt-6 scrollbar-hide"
        style={{ height: 'calc(100vh - 70px)', paddingTop: isOnline ? '1.5rem' : '2.5rem' }}
      >
        {(() => {
          switch (view) {
            case 'dashboard':
            ...
            case 'budget':
              return <BudgetCalculator inventory={inventory} />;
            case 'chat':
              return (
                <ChatInterface
                  inventory={inventory}
                  ...
                  boxCounts={boxCounts}
                  updateInventory={updateInventoryBatch}
            ...
            case 'admin':
              return (
                <AdminPanel
                  tasks={adminTasks}
                  onToggleTask={toggleAdminTask}
                  updateTaskDate={updateAdminTaskDate}
                  user={user}
                  goBack={() => setView('dashboard')}
            ...
            case 'moving':
              return (
                <MovingAssistant
                  ...
                  setFurnitureVolume={setFurnitureVolume}
                />
              );
            case 'social-report':
              return (
            ...
            case 'marketplace':
              return <Marketplace user={user} goBack={() => setView('dashboard')} />;
            case 'templates':
              return (
                <CommunityTemplates
                  goBack={() => setView('dashboard')}
            ...
            case 'daily-groceries':
              return <DailyGroceries goBack={() => setView('dashboard')} />;
            default:
              return null;
          }
        })()}
          ...
          <nav className="absolute bottom-0 left-0 right-0 bg-white dark:bg-gray-800 border-t border-gray-200 dark:border-gray-700 h-[80px] pb-4 flex justify-around items-center z-40 transition-colors">
            ...
            <div className="relative -top-5">
              <button
                ...
                className={`w-14 h-14 rounded-full flex items-center justify-center shadow-lg transition-transform hover:scale-105 ${view === 'chat' ? 'bg-indigo-700 text-white ring-4 ring-indigo-100 dark:ring-indigo-900' : 'bg-indigo-600 text-white'}`}
              >
